...
export function TimelineHero({ babyId, entry, authorName = '家人', mediaIds = [] }: TimelineHeroProps) {
  if (!entry) {
    return (
      <section className="bl-timeline-hero rounded-[var(--radius-hero)] border-2 border-dashed border-[var(--color-border-light)] bg-[var(--color-surface-2)] px-[var(--space-5)] py-[var(--space-6)] text-center">
        <p className="text-[var(--text-lg)] font-bold text-[var(--color-fg-strong)]">今天还没有记录</p>
        <p className="mt-[var(--space-1)] text-[var(--text-sm)] text-[var(--color-fg-soft)]">写下一个小瞬间,以后会很珍贵。</p>
  ...
  if (hasMedia) {
    return (
      <Link href={`/entry/${entry.id}`} className="bl-timeline-hero group relative block h-[200px] overflow-hidden rounded-[var(--radius-hero)] bg-[var(--color-surface)]">
        ...
        {mediaIds.length > 1 && (
          <span className="absolute right-[var(--space-3)] top-[var(--space-3)] rounded-[var(--radius-sm)] bg-[var(--color-media-badge)] px-[10px] py-[var(--space-1)] text-[var(--text-xs)] font-bold text-[var(--color-fg-inverse)]">
            {mediaIds.length} 张
          </span>
        )}
  ...
  return (
    <Link href={`/entry/${entry.id}`} className="bl-timeline-hero block rounded-[var(--radius-hero)] bg-[var(--color-surface)] px-[var(--space-5)] py-[var(--space-5)]">
      <p className="line-clamp-3 whitespace-pre-wrap text-[var(--text-md)] font-bold leading-[var(--leading-base)] text-[var(--color-fg-strong)]">{entry.content}</p>
      <p className="mt-[var(--space-3)] text-[var(--text-xs)] font-semibold text-[var(--color-fg-soft)]">
...
function HeroText({ entry, authorName, inverse = false }: { entry: TimelineHeroEntry; authorName: string | null; inverse?: boolean }) {
  return (
    <span className="absolute inset-x-0 bottom-0 z-10 block p-[var(--space-5)]">
      <span className={inverse ? 'line-clamp-2 text-[var(--text-md)] font-bold leading-[var(--leading-base)] text-[var(--color-fg-inverse)]' : 'line-clamp-2 text-[var(--text-md)] font-bold leading-[var(--leading-base)] text-[var(--color-fg-strong)]'}>
        {entry.content}
      </span>
      <span className={inverse ? 'mt-[var(--space-1)] block text-[var(--text-xs)] font-semibold text-white/85' : 'mt-[var(--space-1)] block text-[var(--text-xs)] font-semibold text-[var(--color-fg-soft)]'}>
        {authorName} · {formatTime(entry.occurredAt)}
      </span>
    </span>
  );
