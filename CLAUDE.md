...
- **Media pipeline** — `app/api/media/` uses `formidable` for multipart upload, then `sharp` (image variants) and ffmpeg (`fluent-ffmpeg`, video probe + covers); records land in `media` + `entryMedia`. Size limits (`media.max*Bytes`) are enforced server-side (413 on exceed). **ffmpeg binary resolution** lives in `lib/server/media/ffmpeg-paths.ts`: dev falls back to the `ffmpeg-static`/`ffprobe-static` packages, but those prebuilt binaries are glibc-linked and aren't bundled into the Next standalone output, so they fail on the Alpine (musl) container — every video upload then errors at probe/derive while images still work. The Docker image therefore installs system ffmpeg (`apk add ffmpeg`) and points the app at it via `FFMPEG_PATH`/`FFPROBE_PATH`. Don't revert the media code to import the static paths directly.
