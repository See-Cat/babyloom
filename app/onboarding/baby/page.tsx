  ...
  return (
    <main className="min-h-screen bg-[var(--color-bg)]">
      ...
      <form action={onSubmit} className="mx-auto flex w-full max-w-md flex-col px-[var(--space-6)] pt-[var(--space-8)]">
        <div className="mb-[var(--space-5)] flex items-center justify-center gap-[var(--space-2)] text-[var(--text-xs)] font-bold uppercase tracking-[0.04em] text-[var(--color-fg-soft)]">
        ...
        <div className="mb-[var(--space-4)] text-center">
          <h1 className="text-[var(--text-2xl)] font-bold text-[var(--color-fg-strong)]">第一个宝宝</h1>
          <p className="text-[var(--text-base)] text-[var(--color-fg-soft)]">先添加一个宝宝,就可以开始记录啦</p>
        </div>
        <div className="mb-[var(--space-4)] flex flex-col items-center gap-[var(--space-2)]">
          <span className="flex h-[88px] w-[88px] items-center justify-center rounded-full bg-[var(--color-avatar-pink)] text-[34px] font-bold text-[var(--color-fg-inverse)]">
            {initialFor(baby.name)}
          </span>
          <p className="text-center text-[var(--text-xs)] font-medium text-[var(--color-fg-soft)]">头像可稍后在宝宝管理里设置</p>
        </div>
        <div className="flex flex-col gap-[var(--space-4)]">
          ...
          {error && <p role="alert" className="rounded-[var(--radius-sm)] bg-[var(--color-error-bg)] px-[var(--space-4)] py-[var(--space-3)] text-[var(--text-sm)] font-semibold text-[var(--color-error-active)]">{error}</p>}
