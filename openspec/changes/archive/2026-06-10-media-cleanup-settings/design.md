...
`runReconcileOnce` reads `{ enabled, thresholdHours }` at the start of each run. The orphan-cleanup step is gated on `mode === 'manual' || enabled` (see D5) and uses `thresholdHours`; after it runs, it records `lastRunAt` + `deletedCount`. The internal hygiene (stuck-`pending`/`processing` recovery + staging GC) is unconditional **with respect to the enabled flag**, but is a background concern that runs **only on the scheduled tick** (`mode === 'scheduled'`) — a manual run does not perform it (see D5). Because cadence is not configurable, the existing `setInterval(24h)` scheduler is kept as-is — no scheduler refactor. A disabled→enabled toggle therefore takes effect at the next scheduled tick (or immediately via run-now).
...
A manual run is an explicit owner action and executes one cleanup pass regardless of `enabled`; scheduled runs respect `enabled`. To express this WITHOUT duplicating cleanup logic in the route (which would drift from the worker), `runReconcileOnce` takes `{ mode: 'scheduled' | 'manual' }` (default `'scheduled'`). The orphan-cleanup step runs when `mode === 'manual' || enabled`. The run-now route delegates to `runReconcileOnce({ mode: 'manual' })` — one cleanup primitive, two entry points. Manual still honors the higher-tier guards (env kill-switch → 503 at the route; backup barrier).

**Manual is scoped to the orphan-draft cleanup ONLY.** The owner panel is presented as "草稿照片清理", so a manual run performs only the orphan-draft soft-delete + stat update; the internal hygiene (stuck-`pending`/`processing` recovery + staging GC) stays on the scheduled tick. This keeps the user-triggerable button from acting as a destructive maintenance pass — e.g. recovering another member's still-`processing` upload to `failed` and then purging its staging dir on slow self-hosted hardware. The hygiene is harmless background work either way, but it is not what the panel advertises, and (unlike orphan cleanup) gains nothing from being on-demand. Mode-awareness lives inside the one primitive — still no route-side duplication. Considered a separate `forceOrphanCleanup` boolean; the `mode` enum reads clearer at both call sites and leaves room for future modes.
