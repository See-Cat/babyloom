# media-cleanup-settings Specification

## Purpose

Owner-controllable, persisted runtime settings governing the orphan-media cleanup performed by the reconcile worker — enabling/disabling cleanup, configuring the unsaved-draft age threshold, recording run statistics, and exposing manual run-now and eligible-orphan preview to the family owner, all while respecting the env kill-switch and backup write barrier.
